...
const API_PROXY_URL = '/api/wordpress-proxy';
// Use the proxy URL for client-side requests
const WORDPRESS_API_URL = process.env.NEXT_PUBLIC_WORDPRESS_API_URL || '/api/wordpress-proxy';
// Direct URL for server-side requests - using HTTPS consistently
const WORDPRESS_DIRECT_API_URL = process.env.WORDPRESS_DIRECT_API_URL || 'https://wp.ajyxn.com/wp-json';
...
function buildProxyUrl(path) {
  if (typeof window !== 'undefined') {
    // Client-side: Use the full URL with searchParams
    const url = new URL(API_PROXY_URL, window.location.origin);
    url.searchParams.set('path', path);
    return url.toString();
  }

  // Server-side: Use the correct query parameter format
  return `${WORDPRESS_API_URL}?path=${encodeURIComponent(path)}`;
}
...
export async function getPropertyById(id, skipCache = false) {
  try {
    ...
    try {
      ...
      // Try direct CORS proxy first for better reliability
      ...
      console.log(`Trying CORS proxy first: ${corsProxyUrl}`);
      ...
      try {
        ...
        console.log(`CORS proxy response time: ${Date.now() - corsStartTime}ms, status: ${corsResponse.status}`);

        if (corsResponse.ok) {
          const property = await corsResponse.json();

          if (property && property.id) {
            console.log(`Successfully fetched property ${id} via CORS proxy`);
            return property;
          }

          console.log(`CORS proxy returned invalid property data for ID: ${id}`);
        } else {
          console.log(`CORS proxy returned error status: ${corsResponse.status}`);
        }
      } catch (corsError) {
        console.error(`Error using CORS proxy: ${corsError.message}`);
      }

      // If CORS proxy fails, try WordPress proxy
      console.log(`Falling back to WordPress proxy for ID: ${id}`);
      ...
      console.log(`WordPress API response time: ${Date.now() - startTime}ms, status: ${response.status}`);
      ...
      console.log(`Successfully fetched property ${id} via WordPress proxy`);
