...
const WORDPRESS_API_URL = process.env.NEXT_PUBLIC_WORDPRESS_API_URL || 'http://anjia.local/wp-json';
...
export async function getPropertiesWithFilters(options = {}) {
  ...
  try {
    ...
    // Cache the transformed properties
...
const DESCRIPTION_CACHE_DURATION = 1000 * 60 * 60 * 24; // 24 hours
...
async function enhanceDescription(description) {
  ...
  // Generate a cache key based on the description content
  const cacheKey = description.trim().substring(0, 100); // Use first 100 chars as key
  ...
  if (cachedDescription && (Date.now() - cachedDescription.timestamp < DESCRIPTION_CACHE_DURATION)) {
    console.log('Using cached enhanced description');
    return cachedDescription.content;
  }

  // If description is already long enough, skip enhancement to save API calls
  if (description.length > 500) {
    console.log('Description already detailed, skipping enhancement');
    return description;
  }

  try {
    console.log('Enhancing property description with Deepseek API');
...
export async function transformPropertyData(wpProperty, skipEnhancement = false) {
  try {
    ...
    // Only enhance description if needed and not skipped
    const description = skipEnhancement
      ? originalDescription
      : await enhanceDescription(originalDescription);
    ...
    const transformedProperty = {
      ...
      // Ensure amenities are always fetched from backend
      amenities: Array.isArray(acf.amenities) && acf.amenities.length > 0
        ? acf.amenities
        : (typeof acf.amenities === 'string' && acf.amenities.trim() !== ''
          ? acf.amenities.split(',').map(a => a.trim())
          : ["WiFi", "Parking", "Security", "Swimming Pool", "Gym"]),
